...
const AdminWritingPage = () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.isAdmin) {
            ...
            const { checkWritingMigrationStatus } = await import('@/lib/actions/admin.actions');
            const migrationResult = await checkWritingMigrationStatus();
  ...
  const handleMigration = async () => {
    if (!confirm('Are you sure you want to migrate all writing test JSON files to Firestore? This action cannot be undone.')) {
      return;
    }

    try {
      setMigrating(true);
      const { migrateWritingTests, checkWritingMigrationStatus } = await import('@/lib/actions/admin.actions');
      const result = await migrateWritingTests();
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b shadow-sm">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Admin Panel - Writing Tests</h1>
            <p className="text-gray-600 mt-1">Manage IELTS writing tests and migrate data</p>
          </div>
          <button
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="bg-yellow-50 border-2 border-yellow-200 rounded-lg p-6 mb-8">
            <div className="flex items-start">
              ...
              <div className="flex-1">
                ...
                >
                  {migrating ? 'Migrating...' : 'Migrate Writing Tests to Firestore'}
                </button>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-green-700">
                  All {migrationStatus.firestoreDocuments} writing tests are successfully stored in Firestore.
                </p>
              </div>
            </div>
          </div>
        ...
        {/* Test Manager */}
        <TestManager testType="writing" isAdmin={isAdmin} />
      </div>
    </div>
  );
};
